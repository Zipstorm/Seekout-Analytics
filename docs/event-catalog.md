...
All hiring surface events include `job_id`. The user's persona context is available via the `current_persona` person property (`$set`) — no need to pass it per-event. See [Schema](./event-schema.md) for standard event properties.
...
| `current_persona`       | enum | person ($set)             | `hiring_manager`, `recruiter`, `job_seeker`                                         | `identifyUser()`, persona switch (replaces `role`, `surface`) |
