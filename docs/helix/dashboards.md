...
- Signup conversion funnel: Page Viewed (`auth_signup`) → Login Started Button Clicked (`auth_signup`) → Auth Login Succeeded → Onboarding Persona Card Clicked → Account Create Succeeded → Onboarding Complete Succeeded. **Live in PostHog** as "New-User Onboarding Funnel" on the Adoption & Engagement dashboard — see detailed filter spec above. **Identity stitching required:** `posthog.reset()` on auth page mount + `posthog.alias()` before `posthog.identify()` — see event-schema.md PostHog Identity Lifecycle. Do NOT filter by `helix_session_id` across the auth boundary (steps 1–3 are pre-auth).
