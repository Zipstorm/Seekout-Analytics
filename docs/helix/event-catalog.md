...
| ----------------- | ---------- | ----------- | -------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----- | ----------------------------------------------------------------------------- | ------ |
| Page Viewed       | Navigation | View   | User lands on a meaningful page                                | Frontend | `current_page_context`, `previous_page_context`, `entry_point` (login page only)                                                        | --    | `$set_once: entry_point, first_referrer, first_landing_url` (login page only) | Live   |
| Login Started     | Account    | Started | User clicks "Continue with Google or Email" on auth landing    | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entry_point`, `entity_type`, `component`                    | --    | --                                                                            | Live   |
| Login Cancelled   | Account    | Success | User closes MSAL popup without completing auth                 | Frontend | `auth_mode`, `error_code`                                                                                                               | --    | --                                                                            | Live   |
| Account Created   | Account    | Success | User clicks "Continue as [Persona]" and server confirms role   | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`, `first_persona`, `auth_method`, `referred_by` | --    | `$set: current_persona, activated_personas`; `$set_once: first_persona, account_created_at, referred_by` | Live   |
| Intro Completed   | Account    | Success | User clicks "Let's go" on onboarding intro page               | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`, `auth_method`                    | --    | --                                                                            | Live   |
...
| Auth Login Failed              | Account | Rejected   | Backend returns auth error                     | Frontend | `auth_mode`, `status_code`, `error_detail`     | --    | --               | Live (legacy) |
...
| Auth Session Restore Failed    | Account | Rejected   | Session restore failed                         | Frontend | `status_code`                                  | --    | --               | Live (legacy) |
| Auth Refresh Failed            | Account | Rejected   | Token refresh failed                           | Frontend | `source`, `status_code`                        | --    | --               | Live (legacy) |
| Auth Logout Completed          | Account | Success   | User logs out                                  | Frontend | `auth_mode`                                    | --    | --               | Live (legacy) |
...
| Event                              | Area    | Type | Trigger                                | Source   | Properties                          | Group | Property Updates | Status |
| ---------------------------------- | ------- | ---- | -------------------------------------- | -------- | ----------------------------------- | ----- | ---------------- | ------ |
| Auth Email Verify Code Sent        | Account | Success   | Verification code sent to user's email | Frontend | `cooldown_seconds`                  | --    | --               | Live   |
| Auth Email Verify Code Send Failed | Account | Rejected   | Failed to send verification code       | Frontend | `status_code`, `error_detail`       | --    | --               | Live   |
| Auth Email Verify Resend Clicked   | Account | Interaction   | User clicks resend verification link   | Frontend | --                                  | --    | --               | Live   |
| Auth Email Verified                | Account | Success   | Email verification succeeds            | Frontend | --                                  | --    | --               | Live   |
| Auth Email Verify Failed           | Account | Rejected   | Email verification fails               | Frontend | `status_code`, `error_detail`, `attempts_remaining` | --    | --               | Live   |
...
| Event                     | Area    | Type | Trigger                       | Source   | Properties                    | Group | Property Updates | Status |
| ------------------------- | ------- | ---- | ----------------------------- | -------- | ----------------------------- | ----- | ---------------- | ------ |
| Auth Phone Submitted      | Account | Success   | User submits phone number     | Frontend | `phone_length`, `country_code` | --    | --               | Live   |
| Auth Phone Submit Failed  | Account | Rejected   | Phone submission fails        | Frontend | `status_code`, `error_detail`  | --    | --               | Live   |
| Auth Phone Skipped        | Account | Success   | User skips phone collection   | Frontend | --                             | --    | --               | Live   |
...
| Event                           | Area     | Type    | Trigger                                     | Source   | Properties                                                                                             | Group | Property Updates | Status      |
| ------------------------------- | -------- | ------- | ------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------ | ----- | ---------------- | ----------- |
| Resume Upload Button Clicked    | Prospect | Interaction  | User clicks resume upload area              | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`  | --    | --               | Live        |
| Resume Uploaded                 | Prospect | Success      | Backend confirms resume stored              | Backend  | `resume_id`, `resume_name`, `resume_file_type`, `resume_size_bytes`, `page_count`                      | --    | --               | Live        |
| Resume Upload Failed            | Prospect | Rejected     | Backend rejects resume                      | Backend  | `resume_file_type`, `resume_size_bytes`, `error_reason`, `error_category`                              | --    | --               | Live        |
| Resume Removed                  | Prospect | Interaction  | User clicks Remove on resume                | Frontend | `action`, `action_value`, `resume_id`, `resume_file_type`, `current_page_context`, `previous_page_context`, `component`, `entity_type` | --    | --               | Live        |
| LinkedIn Export Learn How Link Clicked | Prospect | Interaction | User clicks "Learn how" link           | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`  | --    | --               | Live        |
| Add Profile Photo Button Clicked | Prospect | Interaction | User opens photo action menu                | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`  | --    | --               | Live        |
| Profile Photo Added             | Prospect | Success      | Photo saved successfully                    | Frontend | `upload_method`, `file_type`, `file_size_bytes`, `current_page_context`, `previous_page_context`, `entity_type` | --    | --               | Live        |
| Profile Photo Upload Failed     | Prospect | Rejected     | Photo rejected (size/format)                | Frontend | `upload_method`, `file_size_bytes`, `error_reason`, `error_category`, `current_page_context`, `previous_page_context`, `entity_type` | --    | --               | Live        |
| Profile Photo Removed           | Prospect | Interaction  | User clicks Remove on photo                 | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `component`, `entity_type`  | --    | --               | Live        |
| Build Profile Button Clicked    | Prospect | Interaction  | User clicks "Build my AI profile"           | Frontend | `action`, `action_value`, `current_page_context`, `previous_page_context`, `entity_type`, `component`  | --    | --               | Live        |
| Build Profile Snapshot          | Prospect | Success      | State capture before AI generation          | Frontend | `has_resume`, `resume_id`, `resume_file_type`, `resume_page_count`, `has_photo`, `photo_upload_method`, `links_count`, `link_types`, `current_page_context`, `previous_page_context`, `entity_type` | --    | --               | Live        |
| Candidate Profile Created       | Prospect | Success      | Backend generates portfolio                 | Backend  | `portfolio_id`, `resume_id`, `input_method`, `has_resume`, `has_photo`, `links_count`, `link_types`    | --    | --               | Live        |
| Candidate Profile Creation Failed | Prospect | Rejected   | Backend portfolio generation fails          | Backend  | `resume_id`, `error_reason`, `error_category`                                                          | --    | --               | Live        |
| Profile Section Updated         | Prospect | Success      | User edits a profile section                | Frontend | `section`                                                                                              | --    | --               | Not Started |
| Custom Link Added               | Prospect | Success      | User creates a named shareable link         | Backend  | `current_persona`, `link_type`, `link_name`, `is_job_specific` (boolean), `target_job_title` (optional), `target_company` (optional) | --    | --               | Live        |
...
| `current_persona`       | enum | event, person ($set)      | `hiring_manager`, `recruiter`, `job_seeker`, `unknown`                              | Switch Persona Button Clicked, Persona Updated, Account Created (person property), Auth Login Succeeded (via `identifyUser()`), Auth Session Restore Succeeded (via `identifyUser()`), Custom Link Added |
...
| `input_method`          | enum | event                     | `resume_upload`, `linkedin_import`                                                  | Candidate Profile Created                                      |
...
| `error_category`        | enum | event                     | `network`, `permission`, `validation`, `server`, `timeout`, `hardware`, `unknown`, `unsupported_format`, `extraction_failed`, `invalid_magic_bytes`, `size_limit`, `ai_generation` | All failure events (incl. Persona Update Failed), Resume Upload Failed, Profile Photo Upload Failed, Candidate Profile Creation Failed |
...
| `upload_method`         | enum | event                     | `take_photo`, `upload`                                                              | Profile Photo Added, Profile Photo Upload Failed               |
| `resume_file_type`      | enum | event                     | `pdf`, `doc`, `docx`, `txt`                                                         | Resume Uploaded, Resume Upload Failed, Resume Removed, Build Profile Snapshot |
| `file_type` (photo)     | enum | event                     | `jpg`, `png`, `gif`, `webp`                                                         | Profile Photo Added                                            |
| `photo_upload_method`   | enum | event                     | `take_photo`, `upload`                                                              | Build Profile Snapshot                                         |
| `link_type`             | enum | event                     | `general`, `job_specific`                                                           | Custom Link Added                                              |
...
| `link_types`         | array | event                | Platform types of added links (`github`, `linkedin`, `portfolio`, `personal_website`, `other`) | Build Profile Snapshot, Candidate Profile Created |
...
| ------------------------- | ------- | ----------- | ------------------------------------------ |
| `is_job_specific`         | boolean | event       | Custom Link Added, Custom Link Shared      |
| `has_custom_link`         | boolean | event       | Interest Expressed                         |
| `has_resume`              | boolean | event       | Interest Expressed, Build Profile Snapshot, Candidate Profile Created |
...
| `has_photo`               | boolean | event       | Build Profile Snapshot, Candidate Profile Created |
...
| `resume_size_bytes`      | number | event | Resume file size in bytes                      | Resume Uploaded, Resume Upload Failed    |
| `file_size_bytes`        | number | event | Image file size (photo events only)            | Profile Photo Added, Profile Photo Upload Failed |
| `page_count`             | number | event | Page count (PDF native, DOCX via page breaks, TXT estimated; null for .doc or on error) | Resume Uploaded                          |
| `links_count`            | number | event | Total external links                           | Build Profile Snapshot, Candidate Profile Created |
| `resume_page_count`      | number | event | Resume page count in Build Profile Snapshot    | Build Profile Snapshot                   |
...
| `current_page_context`| string   | event              | Page where event occurred (snake_case, `_` hierarchy)                      | Page Viewed, Login Started, Account Created, Intro Completed, Switch Persona Button Clicked, Sam Session Started, Sam Session Ended, all Job Post Wizard events, Resume Upload Button Clicked, Resume Removed, LinkedIn Export Learn How Link Clicked, Add Profile Photo Button Clicked, Profile Photo Added, Profile Photo Upload Failed, Profile Photo Removed, Build Profile Button Clicked, Build Profile Snapshot |
...
| `component`           | string   | event              | UI container where action occurred                                         | Login Started, Account Created, Intro Completed, Switch Persona Button Clicked, all Job Post Wizard step events, Resume Upload Button Clicked, Resume Removed, LinkedIn Export Learn How Link Clicked, Add Profile Photo Button Clicked, Profile Photo Removed, Build Profile Button Clicked |
| `entity_type`         | string   | event              | Business object being acted on                                             | Login Started, Account Created, Intro Completed, Switch Persona Button Clicked, all Job Post Wizard step events, Resume Upload Button Clicked, Resume Removed, LinkedIn Export Learn How Link Clicked, Add Profile Photo Button Clicked, Profile Photo Added, Profile Photo Upload Failed, Profile Photo Removed, Build Profile Button Clicked, Build Profile Snapshot |
| `link_name`           | string   | event              | User-provided name for the custom link                                     | Custom Link Added                                |
| `target_job_title`    | string   | event              | Job title the link targets (optional, only when `is_job_specific` is true) | Custom Link Added                                |
| `target_company`      | string   | event              | Company the link targets (optional, only when `is_job_specific` is true)   | Custom Link Added                                |
| `resume_id`           | string   | event              | Resume identifier                                                          | Resume Uploaded, Resume Removed, Build Profile Snapshot, Candidate Profile Created, Candidate Profile Creation Failed |
| `resume_name`         | string   | event              | Original uploaded filename                                                 | Resume Uploaded                                  |
| `portfolio_id`        | string   | event              | Generated portfolio identifier                                             | Candidate Profile Created                        |
| `reason`              | string   | event              | User-provided reason for withdrawal                                        | Interest Withdrawn                               |
| `error_reason`        | string   | event              | System error description                                                   | All failure events, Resume Upload Failed, Profile Photo Upload Failed, Candidate Profile Creation Failed |
